...
export default class BehavioralTracker {
  ...
  _onMouseMove(e) {
    ...
    if (this.lastPos) {
      ...
      if (dist > 2) {
        ...
        if (
          this.lastDirection != null &&
          Math.abs(direction - this.lastDirection) > Math.PI / 4
        ) {
  ...
  _flushTouch(step) {
    ...
    const avgDuration = this.touch.durations.length
      ? this.touch.durations.reduce((a, b) => a + b, 0) /
        this.touch.durations.length
      : 0;
    ...
    const onClick = () => {
      if (this._confirmHoverStart != null) {
        this.trackEvent("confirm_hover", {
          hover_ms_before_click: Math.round(
            performance.now() - this._confirmHoverStart,
          ),
        });
      }
    };
  ...
  attachKeystrokeTiming(el, fieldName) {
    ...
    const onBlur = () => {
      if (state.intervals.length >= 2) {
        ...
        const variance =
          state.intervals.reduce((a, b) => a + (b - mean) ** 2, 0) /
