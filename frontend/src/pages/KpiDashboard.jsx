...
function StatCard({ label, value, supporting }) {
  return (
    <Card>
      ...
      <p className="font-sans text-[26px] leading-[32px] font-semibold text-ink-900 mb-1">{value}</p>
            ...
            <div style={{ width: '100%', height: 260 }}>
              <ResponsiveContainer>
                <BarChart data={chartData} barSize={40}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#EBEBEE" vertical={false} />
                  <XAxis
                    dataKey="decision"
                    stroke="#63636D"
                    fontSize={12}
                  ...
                  <Bar dataKey="count" radius={[6, 6, 0, 0]}>
                    {chartData.map((entry) => (
                      <Cell key={entry.decision} fill={RISK_COLORS[entry.decision] || '#23265C'} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
