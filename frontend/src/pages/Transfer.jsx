...
export default function Transfer() {
  ...
  const [showGeoConsent, setShowGeoConsent] = useState(() => {
    return (
      !sessionStorage.getItem("latchpoint_geo_prompted") &&
  ...
  useEffect(() => {
    if (step === "form" && amountInputRef.current) {
      trackerRef.current?.attachKeystrokeTiming(
  ...
  async function handleVerdict(result) {
    ...
    if (result.calibrating) {
      setCalibratingInfo(
        result.calibration_progress || { current: 1, total: 10 },
      );
      const confirmRes = await api.post(
        `/transactions/${result.transaction_id}/confirm`,
      );
      await refreshUser();

      if (
        confirmRes?.completed_calibration ||
        result.will_complete_calibration
      ) {
