...
import { FileSearch, ArrowUpRight, CheckCircle2, AlertTriangle, XCircle, Clock, Send } from "lucide-react";
...
export default function AdminInvestigations() {
  ...
  async function handleCaseAction(action) {
    ...
    try {
      await api.post(`/admin/commitments/${selectedCase.commitment_id}/action`, { action });
      loadCases();
    } catch {
      // best effort
    }
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-secondary text-[#8E96A8] text-xs mt-0.5">
              Deep forensic review of high-consequence pre-commitment holds and evidence packages
            </p>
          </div>
        </div>
        ...
        {decisionFeedback && (
          <div className="p-3 bg-[#1B2538] border border-[#2B3958] rounded text-xs text-white flex items-center justify-between">
            ...
            <button onClick={() => setDecisionFeedback("")} className="text-[#8E96A8] hover:text-white">✕</button>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-[#141722] border border-[#222738] rounded-md p-4 space-y-3">
            <div className="flex items-center justify-between pb-2 border-b border-[#222738]">
              ...
              <span className="text-[10px] text-amber-400 font-mono">PRIORITY 1</span>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between text-xs">
                      <span className="font-mono font-semibold text-[#8E9BFF]">{c.case_id}</span>
                      <span className="font-mono text-rose-400 font-semibold">{c.risk_score}/100</span>
                    </div>
                    <div className="font-medium text-white text-xs mt-1">{c.user_name}</div>
                    <div className="flex items-center justify-between text-[11px] text-[#8E96A8] mt-1">
                      <span>Amount: ₹{c.amount.toLocaleString()}</span>
                      <span className="font-mono uppercase text-amber-400 font-semibold">{c.decision}</span>
                    </div>
                  </div>
                );
              })}
            ...
            {selectedCase ? (
              <>
                ...
                <div className="bg-[#141722] border border-[#222738] rounded-md p-5 flex flex-wrap items-center justify-between gap-4">
                  <div>
                    <span className="text-[10px] uppercase tracking-wider text-[#6E7891] font-semibold">Active Investigation</span>
                    <h2 className="text-lg font-bold text-white font-mono mt-0.5">{selectedCase.case_id}</h2>
                    <p className="text-xs text-[#8E96A8]">{selectedCase.user_name} ({selectedCase.user_id})</p>
                  </div>

                  <div className="flex items-center gap-4 text-xs">
                    <div>
                      <span className="text-[#6E7891] text-[10px] uppercase">Commitment Hold</span>
                      <div className="text-base font-bold font-mono text-white mt-0.5">₹{selectedCase.amount.toLocaleString()}</div>
                    </div>

                    <div>
                      <span className="text-[#6E7891] text-[10px] uppercase">Risk Score</span>
                      <div className="text-base font-bold font-mono text-rose-400 mt-0.5">{selectedCase.risk_score}/100</div>
                    </div>
                ...
                <div className="bg-[#141722] border border-[#222738] rounded-md p-4 space-y-3">
                  ...
                  <ul className="space-y-2 text-xs">
                    {selectedCase.evidence.map((ev, i) => (
                      <li key={i} className="flex items-start gap-2 bg-[#1A1E2B] p-2.5 rounded border border-[#262C3E] text-[#B0B8C8]">
                ...
                <div className="bg-[#141722] border border-[#222738] rounded-md p-4 space-y-4">
                  ...
                  <div>
                    <label className="text-[11px] text-[#8E96A8] block mb-1">Analyst Case Notes & Rationalization</label>
                    <textarea
                      rows={3}
                      value={analystNote}
              ...
              <div className="py-20 text-center text-xs text-[#6E7891]">Select a case to inspect.</div>
            )}
          </div>
        </div>
      </div>
